...
import emu.grasscutter.command.parser.annotation.CommandDescription;
...
public class TargetCommand {
    @DefaultHandler
    @CommandDescription("commands.target.description")
    public void setTarget(BaseCommandSource source, @OptionalArgument Integer targetUid) {
        ...
        if (targetUid == null) {
            source.showMessage(translate("commands.execution.clear_target"));
            return;
        }
        source.pushPrompt("Target: %d".formatted(targetUid));
        source.showMessage(translate("commands.execution.set_target", targetUid.toString()));
    }
